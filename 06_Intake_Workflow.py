# 06_Intake_Workflow.py — simple, focused intake flow without sidebar nav polish
...
# Local storage for in-progress intake entries
...
with st.container(border=True):
    ...
    with c2:
        draft["contact"]["last_name"] = st.text_input("Last name", value=draft["contact"]["last_name"] or (lead["name"].split()[1] if len(lead["name"].split())>1 else ""))
...
with st.container(border=True):
    ...
    current = draft["care_needs"].get("adls") or []
    selected = st.multiselect("ADLs", adl_opts, default=[a for a in current if a in adl_opts])
...
actions = st.columns([0.3,0.3,0.4])
with actions[0]:
...
st.caption("This workflow is reachable via Start Intake; it can be hidden from the sidebar in a future Streamlit navigation upgrade.")
