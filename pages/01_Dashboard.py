...
# Use raw HTML <h2> instead of Markdown ## to avoid Streamlit’s default divider
st.markdown("<h2 style='margin-bottom:0;'>Advisor Dashboard</h2>", unsafe_allow_html=True)

# At-a-glance section
with st.container():
    st.markdown("<div class='glance-card'>", unsafe_allow_html=True)
    ...
    with col1:
        st.markdown(
            "<div class='kpi'><span class='kpi-label'>New leads (today)</span>"
            "<span class='kpi-value'>5</span>"
            "<span class='pill pill-good'>+2 vs yesterday</span></div>",
            unsafe_allow_html=True
        )
    with col2:
        st.markdown(
            "<div class='kpi'><span class='kpi-label'>Assigned leads</span>"
            "<span class='kpi-value'>12</span>"
            "<span class='pill pill-good'>+1 this week</span></div>",
            unsafe_allow_html=True
        )
    with col3:
        st.markdown(
            "<div class='kpi'><span class='kpi-label'>Active cases</span>"
            "<span class='kpi-value'>3</span>"
            "<span class='pill pill-bad'>-1 since Fri</span></div>",
            unsafe_allow_html=True
        )
    with col4:
        st.markdown(
            "<div class='kpi'><span class='kpi-label'>MTD vs goal</span>"
            "<span class='kpi-value'>$20,500 / $40,000</span>"
            "<span class='pill pill-warn'>51% of goal</span></div>",
            unsafe_allow_html=True
        )
    st.markdown("</div>", unsafe_allow_html=True)

# Dismissable blue notices
...
for idx, n in enumerate(list(st.session_state["notices"])):
    ...
    with cols[0]:
        st.markdown(
            f"<div class='note'>{n['text']} <span class='small-muted'>[{n['type']}]</span></div>",
            unsafe_allow_html=True
        )
    with cols[1]:
